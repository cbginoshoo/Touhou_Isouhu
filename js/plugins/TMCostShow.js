...
/*:
 * @plugindesc (ver 1.0.0 custom 0.0.1)ＭＰ消費とＴＰ消費が両方設定された
 * スキルのコストを無理やり両方表示します。
 *
 ...
 *
 * @help
 * TMPlugin - コスト表示拡張 ver1.0.0 custom 0.0.1
 *
 * 使い方:
 *
 ...
 *   重なって表示されてしまいます。
 *
 *   このプラグインではコストの文字列数によりスキル名文字列の
 *   描画幅を狭くすることでスキル名とコスト表示を重ねず表示させます。
 *
...
(function() {
  ...
  Window_SkillList.prototype.drawItem = function(index) {
    ...
    if (skill) {
        var costWidth = this.TMcostWidth(skill);
  ...
  Window_SkillList.prototype.skillTpCost = function(tpCost, count) {
    return tpCostHeader + tpCost;
  };
  ...
  Window_SkillList.prototype.skillMpCost = function(mpCost, count) {
    return (count > 0 ? conjunction : "") + mpCostHeader + mpCost;
  };
  ...
  Window_SkillList.prototype.skillHpCost = function(hpCost, count) {
    return (count > 0 ? conjunction : "") + hpCostHeader + hpCost;
  };
  ...
  Window_SkillList.prototype.skillExpCost = function(expCost, count) {
    return (count > 0 ? conjunction : "") + expCostHeader + expCost;
  };
  ...
  Window_SkillList.prototype.skillGoldCost = function(goldCost, count) {
    return (count > 0 ? conjunction : "") + goldCostHeader + goldCost;
  };
  ...
  Window_SkillList.prototype.TMcostWidth = function(skill) {
    var count = 0;
    var text = "";
    var cost = this._actor.skillTpCost(skill);

    if (cost > 0) {
      text += this.skillTpCost(cost, count);
      if (count++ === maxCostNum - 1) {
    ...
    cost = this._actor.skillMpCost(skill);
    if (cost > 0) {
      text += this.skillMpCost(cost, count);
      if (count++ === maxCostNum - 1) {
        return this.textWidth(text);
      }
    }
    if (Imported.TMSkillCostEx) {
      cost = this._actor.skillHpCost(skill);
      if (cost > 0) {
        text += this.skillHpCost(cost, count);
        if (count++ === maxCostNum - 1) {
          return this.textWidth(text);
        }
      }
      cost = this._actor.skillExpCost(skill);
      if (cost > 0) {
        text += this.skillExpCost(cost, count);
        if (count++ === maxCostNum - 1) {
          return this.textWidth(text);
        }
      }
      cost = this._actor.skillGoldCost(skill);
      if (cost > 0) {
        text += this.skillGoldCost(cost, count);
        if (count++ === maxCostNum - 1) {
          return this.textWidth(text);
        }
      }
