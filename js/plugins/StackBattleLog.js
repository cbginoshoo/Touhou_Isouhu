...
Imported['StackBattleLog'] = 1.05;

/*:
 * @plugindesc ver1.05_custom_20200328/戦闘ログを蓄積型に変更し、戦闘ログを表示するパーティコマンドを追加します。
 * @author Yana
 *
 ...
 * @default 100
 *
 * @param Log Clear When End Battle
 * @desc 戦闘終了時に戦闘ログをクリアするかの設定です。
 * @type boolean
 * @on クリアする
 * @off クリアしない
 * @default true
 ...
 * 素材利用は自己責任でお願いします。
 * ------------------------------------------------------
 * ver1.05:Stack Log MaxLinesのパラメータが正常に機能していないバグを修正
...
(function() {
    ...
    var stackLogMaxLines = Number(parameters['Stack Log MaxLines'] || 6);
    var stackLogFontSize = Number(parameters['Stack Log FontSize'] || 24);
    var stackLogSize = Number(parameters['Stack Log Size'] || 24);
    // add
    var stackClearLog = Boolean(parameters['Log Clear When End Battle'] === "true");
    ...
    // add
    Game_Temp.prototype.clearLog = function () {
        this._battleLog = [];
    }
    ...
    Game_Message.prototype.add = function(text, needAddLog){
        if(!needAddLog) {
            needAddLog = false;
        }
        ...
        if ($gameParty.inBattle() && needAddLog) { $gameTemp.addLog(text) }
    };
    ...
    // add
    var _stacklogw_BattleManager_endBattle = BattleManager.endBattle;
    BattleManager.endBattle = function(result) {
        _stacklogw_BattleManager_endBattle.call(this, result);
        if(stackClearLog) {
            $gameTemp.clearLog();
        }
    };
    ...
    BattleManager.displayStartMessages = function() {
        $gameTroop.enemyNames().forEach(function(name) {
            // 出現メッセージ
            $gameMessage.add(TextManager.emerge.format(name), true);
        }, this);
        if (this._preemptive) {
            // 先制メッセージ
            $gameMessage.add(TextManager.preemptive.format($gameParty.name()), true);
        } else if (this._surprise) {
            // 不意打ちメッセージ
            $gameMessage.add(TextManager.surprise.format($gameParty.name()), true);
        }
    };
