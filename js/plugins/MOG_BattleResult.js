...
/*:
 * @plugindesc (v1.2) Modifica a tela de vitoria.
 * @author Moghunter
 *
 * @param Exp X-Axis
 * @desc Posicao X-Axis da EXP.
 * @default 760
 *
 * @param Exp Y-Axis
 * @desc Posicao Y-Axis da EXP.
 * @default 160
 *
 * @param Gold X-Axis
 * @desc Posicao X-Axis do Gold.
 * @default 760
 *
 * @param Gold Y-Axis
 * @desc Posicao Y-Axis do Gold.
 * @default 230
 *
 * @param Treasure X-Axis
 * @desc Posicao X-Axis do tersouro.
 * @default 32
 *
 * @param Treasure Y-Axis
 * @desc Posicao Y-Axis do tesouro.
 * @default 330
 *
 * @param Actor X-Axis
 * @desc Posicao X-Axis do personagem.
 * @default 250
 *
 * @param Actor Y-Axis
 * @desc Posicao Y-Axis do personagem.
 * @default 0
 *
 * @param Parameter Real X-Axis
 * @desc Posicao X-Axis dos parametros.
 * @default 0
 *
 * @param Parameter Real Y-Axis
 * @desc Posicao Y-Axis dos parametros.
 * @default 0
 *
 * @param Parameter 1 X-Axis
 * @desc Posicao X-Axis dos parametros antes do level-up.
 * @default 550
 *
 * @param Parameter 1 Y-Axis
 * @desc Posicao Y-Axis dos parametros antes do level-up.
 * @default 140
 *
 * @param Parameter 2 X-Axis
 * @desc Posicao X-Axis dos parametros depois do level-up.
 * @default 700
 *
 * @param Parameter 2 Y-Axis
 * @desc Posicao Y-Axis dos parametros depois do level-up.
 * @default 140
 *
 * @param New Skill X-Axis
 * @desc Posicao X-Axis da nova habilidade.
 * @default 570
 *
 * @param New Skill Y-Axis
 * @desc Posicao Y-Axis da nova habilidade.
 * @default 520
 *
 * @param Name X-Axis
 * @desc Posicao X-Axis do nome do personagem.
 * @default 155
 *
 * @param Name Y-Axis
 * @desc Posicao Y-Axis do nome do personagem.
 * @default 540
 *
 * @param Name Update Speed
 * @desc Update Animation Speed x N
 * @type number
 * @min 1
 * @max 5
 * @default 3
 ...
 * =============================================================================
 * Modifica a tela de vitoria.
 * =============================================================================
 * Serao necessarios os arquivos. (img/system/)
 *
 * Result_A.png
 * Result_B.png
 * Result_C.png
 ...
 * ACTOR PICTURE
 * =============================================================================
 * As imagens deverao ser gravadas na pasta /img/picture/
 ...
 * HISTORICO
 * =============================================================================
 * (v1.2) - Correcao do plugin parameter nao funcionar.
 * (v1.1) - Correcao de nao apresentar a janela de status na batalha seguinte.
 */
...
//=============================================================================
	...
	var mog_paramParse = function(obj) {
		return JSON.parse(JSON.stringify(obj, mog_paramReplace));
	};

	var mog_paramReplace = function(key, value) {
		try {
	...
	var mog_param = mog_paramParse(PluginManager.parameters('MOG_BattleResult'));
    ...
    Moghunter.bresult_speed = Number(Moghunter.parameters['Name Update Speed'] || 1);
	Moghunter.bresult_lvup_se = mog_param['Level Up Se'];
...
BattleResult.prototype.updateLayoutAnimation = function() {
	...
	this._layout.opacity += 5 * Moghunter.bresult_speed;
	if (this._layout.scale.x > 1.00) {
		// this._layout.scale.x -= 0.05;
		this._layout.scale.x -= 0.05 * Moghunter.bresult_speed;
...
BattleResult.prototype.updateVictAnimation = function() {
	   if (this._phaseAnime[0] === 1) {
		    if (this._vict.scale.x > 1.00) {
				...
				this._vict.scale.x -= 0.05 * Moghunter.bresult_speed;
				this._vict.opacity += 15 * Moghunter.bresult_speed;
				if (this._vict.scale.x <= 1.00) {
		    ...
		    if (this._vict.scale.x < 3.00) {
				...
				this._vict.scale.x += 0.05 * Moghunter.bresult_speed;
				this._vict.opacity -= 10 * Moghunter.bresult_speed;
				if (this._vict.scale.x >= 3.00) {
...
BattleResult.prototype.updateExp = function() {
	 ...
	 var dif_number = this.update_dif(this._expOld,$gameTemp._bResult[1],1);
...
BattleResult.prototype.updateGold = function() {
	 ...
	 var dif_number = this.update_dif(this._goldOld,$gameTemp._bResult[2],1);
...
BattleResult.prototype.updateLevel = function() {
	...
	if (!this._actor && this._actorIndex >= $gameParty._actors.length) {
		this._phase = 5;
		SoundManager.playCursor();
	} else {
		if (this._actor) {
...
BattleResult.prototype.playVoice = function() {
	if (Imported.MOG_BattleCry) {
		SoundManager.selectVoice(this._actor._v_levelup);
	} else {
		if(Moghunter.bresult_lvup_se.name) {
			AudioManager.playSe(Moghunter.bresult_lvup_se);
		}
	}
};
...
BattleResult.prototype.updateLevelPosition = function() {
	...
	if (this._levelLayout.scale.x > 1.00) {
		...
		this._levelLayout.scale.x -= 0.05 * Moghunter.bresult_speed;
...
BattleResult.prototype.updateSkillAnimation = function() {
	...
	// this._skillAnime[0] ++;
	this._skill.opacity += 10 * Moghunter.bresult_speed;
	this._skillAnime[0] += Moghunter.bresult_speed;
	if (this._skillAnime[0] < 20) {
		// this._skill.y -= 2;
		this._skill.y -= 2 + Moghunter.bresult_speed;
...
BattleResult.prototype.sprite_move_to = function(value,real_value,speed) {
	speed = Math.floor(speed / Moghunter.bresult_speed);
	if (value == real_value) {return value};
...
BattleResult.prototype.gainEXP = function() {
	 var actor = $gameActors.actor($gameParty._actors[this._actorIndex]);
