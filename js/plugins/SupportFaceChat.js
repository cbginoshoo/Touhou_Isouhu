...
//=============================================================================
 /*:
 * @plugindesc (v1.22_Custom_v2.0.0) フェイスチャット機能を実装します。
 ...
 *
 * @param FaceChatFinishVariable
 * @desc フェイスチャット閲覧時にコモンイベント番号を記録するための変数を設定できます
 * @type variable
 * @default 0
 *
 * @param FaceChatHelpText
 * @desc フェイスチャットお知らせの下に表示するヘルプのテキストです
 * @default Cキー/タッチでフェイスチャットを起動
 *
 * @help
 *
 * //=============================================================================
 * // 【フェイスチャット補助】　Version: 1.22_Custom_v2.0.0
 * //
 * // ここからリスポーン: http://respawnfromhere.blog.fc2.com/
 * // Twitter: https://twitter.com/ibakip
 * //=============================================================================
 ...
 * 【変更箇所】
 * [プラグインコマンド]
 *   FaceChat addFace フェイス名 顔グラ番号 チャット人数 表示順 不透明度
 *     不透明度パラメータを新規に追加、未指定時は255固定
 *
 *   FaceChat StartBalloon 表示順 align
 *     バルーンのalign調整パラメータを新規に追加。
 *     right / left を指定することで右寄せ / 左寄せができる。未指定時はcenter
 *
 *   FaceChat StartState 表示順 align
 *     サイドビューステートのalign調整パラメータを新規に追加。
 *     right / left を指定することで右寄せ / 左寄せができる。未指定時はcenter
 *
 *
 * 【更新履歴】
 ...
 *
 *  【カスタム履歴】
 *  ○ Ver 1.22_Custom_v2.0.0 (2020/01/02)
 *   ・フェイスチャットの情報を他プラグインへ連携するためのメソッドを追加
 *   ・フェイスチャット履歴プラグイン対応のためisHistoryMode変数を追加
 *   ・isHistoryMode変数による処理分岐を追加
 *
 */
 //=============================================================================
...
// ---プラグインパラメータの取得-----------------------------------------------------
 ...
 SFC_Val.ChatVar = Number(Parameters['FaceChatFinishVariable']) || 0;
 var ChatVar = Number(Parameters['FaceChatFinishVariable']) || 0;

 // フェイスチャットお知らせの下にヘルプを表示
 var FaceChatHelpText = String(Parameters['FaceChatHelpText'] || "%1キーでフェイスチャットを起動");
...
Game_Interpreter.prototype.pluginCommand = function(command, args) {
    ...
    if (command === 'addFaceChat') {
        ...
        if(Imported.FC_FaceChatHistory){
            $gameTemp.deleteFaceChatStatus(args[0]);
        }
    }
        ...
        switch (args[0]) {
            ...
            case 'Finish':
                ...
                if(isHistoryMode) {
                    isHistoryMode = false;
                    historyTitleText = '';
                    $gameMap._interpreter.pluginCommand("facechathistory", ["open"]);
                }
                break;
            case 'addFace':
                draw_Face = true;
                ...
                draw_State = false;
                var tone = [0,0,0,0];
                var opacity = args.length > 4 ? isFinite(args[5]) ? Number(args[5]) : 255 : 255;
                Face[Number(args[4])-1][1] = args[2];
                var facePos = calcXYPosition(args[3],args[4]);
                $gameScreen.showPicture(Number(args[4])+9, args[1], 1, facePos[0], facePos[1], 100, 100, opacity, 0);
                $gameMap._interpreter.wait(3);
                ...
                if(args[1] === 'OFF'){
                    for (var loop=1; loop<=8; loop++) {
                        ...
                        if(pic){
                            ...
                            $gameScreen.movePicture(loop+9, 1, pic.x() , pic.y(), 100, 100, pic.opacity(), 0, 5);
                        }
                    }
                }
                else{
                    for (var loop=1; loop<=8; loop++) {
                        ...
                        if ( pic ){
                            if ( loop !== Number(args[1]) ){
                                ...
                                $gameScreen.tintPicture(loop+9, brightTone, 2);
                                $gameScreen.movePicture(loop+9, 1, pic.x() , pic.y(), 110, 110, pic.opacity(), 0, 5);
                            }
                            if ( pic.scaleX() > 100 ){
                                $gameScreen.movePicture(loop+9, 1, pic.x() , pic.y(), 100, 100, pic.opacity(), 0, 5);
                            }
            ...
            case 'StartBalloon':
                ...
                draw_State = false;
                align = args[3] || "center";
                startBalloonAndState(args[1],args[2],-80, 'Balloon', align);
            ...
            case 'StartState':
                ...
                draw_State = true;
                align = args[3] || "center";
                startBalloonAndState(args[1],args[2],-55, 'States', align);
...
var startBalloonAndState = function(args1, args2, plusY, Name, align){
    align = align || "center";
    ...
    var gamePic = $gameScreen.picture( Number(args1)+17 );
    switch(align) {
        case "right":
            pic.offsetRight();
            gamePic.offsetRight();
...
// Game_Temp
//-----------------------------------------------------------------------------
// フェイスチャットの機能を他プラグインから呼び出すためのメソッド

Game_Temp.prototype.getSeenChatArray = function() {
    if(SFC_Val.ChatVar > 0) {
        return $gameVariables._data[SFC_Val.ChatVar] || [];
    }
    return [];
};

Game_Temp.prototype.callFaceChat = function(id, titleText) {
    ...
    historyTitleText = titleText;
    //let scene = SceneManager._scene;
    //console.log(scene);
    //scene.ChatInfoWindow.hide();
    //scene._mapNameWindow.hide();
};

Game_Temp.prototype.pushFacechatHistory = function(arr) {
    if(!arr || !(arr instanceof Array) || ChatVar < 1) {
        return ;
    }

    var ar = $gameVariables.value(ChatVar);
    if(!ar || !(ar instanceof Array)) {
        ar = new Array();
        $gameVariables._data[ChatVar] = ar;
    }

    $gameVariables._data[ChatVar] = ar.concat(arr);
};

Game_Temp.prototype.distinctFacechatHistory = function() {
    if(ChatVar < 1) {
        return ;
    }
    ...
    var arSet = new Set(ar);
    $gameVariables._data[ChatVar] = Array.from(arSet);
};
...
Scene_Title.prototype.commandNewGame = function() {
    ...
    if(ChatVar > 0) {
        var ar = $gameVariables.value(SFC_Val.ChatVar);
        if(!ar || !(ar instanceof Array)) {
            ar = new Array();
            $gameVariables._data[SFC_Val.ChatVar] = ar;
        }
    }
...
Window_ChatTitle.prototype.refresh = function(){
    if(this.visible){
        ...
        if(isHistoryMode) {
            this.drawText(historyTitleText, 0, y, this.contentsWidth(), 'center' );
        } else {
...
Window_ChatInfo.prototype.refresh = function(){
    ...
    if( FaceChatArray.length !== 0 && FaceChatArray[currentDisplayID] !== null){
        ...
        if( Imported.FC_FaceChatHistory ) {
            titleText = $gameTemp.faceChatTitle(FaceChatArray[currentDisplayID][0], true);
        }
    ...
    // 説明文追加
    var text = FaceChatHelpText;
    this.changeTextColor(this.textColor(6));
    this.contents.fontSize = 18;
    y += 28;
    this.drawText(text, x*2+xPlus, y, this.contentsWidth());
...
// Game_Picture
//-----------------------------------------------------------------------------
var SFC_game_picture_initbasic = Game_Picture.prototype.initBasic;
Game_Picture.prototype.initBasic = function() {
    SFC_game_picture_initbasic.call(this);
    this._offset = {x:0,y:0};
};

Game_Picture.prototype.offsetRight = function() {
    this._offset.x = 43;
};

Game_Picture.prototype.offsetLeft = function() {
    this._offset.x = -43;
};

Game_Picture.prototype.offsetCenter = function() {
    this._offset.x = 0;
};
...
Sprite_Picture.prototype.updatePosition = function() {
    ...
    if(this._balloon){
        ...
        var currentPicture = $gameScreen.picture( this._pictureId );
        this.x = parentPicture._x+currentPicture._offset.x;
        this.y = parentPicture._y-80+currentPicture._offset.y;
    }
    else if(this._state){
        var parentPicture = $gameScreen.picture( this._pictureId-8 );
        var currentPicture = $gameScreen.picture( this._pictureId );
        this.x = parentPicture._x+currentPicture._offset.x;
        this.y = parentPicture._y-55+currentPicture._offset.y;
    }
    else{
