 ...
 *
 * @param CondNonDamage
 * @desc ダメージタイプが[なし,○○回復]のスキルに対して「身代わり」を発動させます。OFFにすると無効になります。(ON/OFF)
 * @default true
 * @type boolean
 *
 ...
 *  ・スキルタイプ「必中」以外
 *
 * 2. 防御、回復、ダメージタイプが[なし]のスキルに対して
 *    「身代わり」を発動させるかどうか
 *
 * 3. 身代わりの詳細な発動条件を細かく設定できます。
...
(function() {
    ...
    param.condNonDamage = getParamBoolean(['CondNonDamage', '身代わり条件_ダメージタイプ']);
    ...
    BattleManager.checkSubstituteDefault = function(target) {
        return this.isValidSubstituteDying(target) && this.isValidSubstituteCertainHit() && this.isValidSubstituteAttack();
    };
    ...
    BattleManager.isValidSubstituteAttack = function() {
        return param.condNonDamage || (!this._action.isGuard() && !this._action.isRecover() && !this._action.checkDamageType([0]));
    };
