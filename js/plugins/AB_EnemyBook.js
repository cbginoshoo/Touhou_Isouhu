...
/*:
 * @plugindesc v1.20 custom1 戦闘中も確認できるモンスター図鑑です。属性、ステートの耐性の確認もできます。
 * @author ヱビ
 *
 ...
 *
 * @param NoName
 * @desc ドロップアイテム/出現場所が設定されていないときに表示する文章です。
 * @default なし
 *
 ...
 * @default 1
 *
 * @param DispUnknownDropItems
 * @type select
 * @option 表示
 * @value 1
 * @option 非表示
 * @value 0
 * @desc 未確認の敵キャラのドロップアイテムを図鑑に未確認状態で表示するか決めます。0:非表示、1:表示
 * @default 1
 *
 ...
 * @option 非表示
 * @value 0
 * @desc 図鑑に敵キャラの出現場所を表示するか決めます。
 * 0:非表示、1:表示
 * @default 1
 *
 * @param DispUnknownPopData
 * @type select
 * @option 表示
 * @value 1
 * @option 非表示
 * @value 0
 * @desc 未確認の敵キャラの出現場所を図鑑に表示するか決めます。
 * 0:非表示、1:表示
 * @default 1
 *
 * @param ---属性アイコン---
 ...
 * @default 6
 *
 * @param PictureFitting
 * @type select
 * @option 調整しない
 * @value 0
 * @desc エネミー画像を文字に被らないように調整するか決めます。
 * @default 0
 *
 * @help
 * ============================================================================
 ...
 * ============================================================================
 *
 * 〇属性の表示方法その1、2通り(UseIconが[使う]のとき)
 *
 * 1.属性の名前の中にアイコンを入れる
 *   例：\i[64]炎
 *
 ...
 *
 * (UseIconが[使う]のとき、名称がアイコンの横に表示されます)
 *
 * 〇未確認の敵キャラ「？？？」
 *
 ...
 * ○敵の出現場所について
 *
 * 敵が出現する場所(マップ)についての情報はdata/EnemyPops.jsonファイルに
 * 記載されています。
 *
 * 敵が増えたり、出現マップを設定した場合はファイル内容の更新が必要です。
 *
 * ============================================================================
 * プラグインコマンド
 * ============================================================================
...
(function() {
	...
	var DispUnknownDropItems = (parameters['DispUnknownDropItems'] == 1) ? true : false;
	var dispRates = [];
	...
	var DropItemName = String(parameters['DropItemName'] || "落とすアイテム");
	var PopMapName = String(parameters['PopMapName'] || "出現場所");
	var NoName = String(parameters['NoName'] || "なし");

	var DispPopData = (parameters['DispPopData'] == 1) ? true : false;
	var DispUnknownPopData = (parameters['DispUnknownPopData'] == 1) ? true : false;
	var UseText = (parameters['UseText'] == 1) ? true : false;

	var StandardFontSize = Number(parameters['StandardFontSize']);
	var StandardPadding = Number(parameters['StandardPadding']);
	var PictureFitting = (parameters['PictureFitting'] == 1) ? true : false;
...
//=============================================================================
	...
	Game_Interpreter.prototype.pluginCommand = function(command, args) {
		...
		if (command === 'EnemyBook') {
			...
			case 'add':
				...
				break;
			case 'addplus':
				$gameSystem.addPlusToEnemyBook(Number(args[1]));
				break;
			case 'remove':
	...
	Game_System.prototype.addPlusToEnemyBook = function(enemyId) {
		if (!this._enemyBookFlags) {
			this.clearEnemyBook();
		}
		this._enemyBookFlags[enemyId] = true;
		this.incrementDefeatNumber(enemyId);
	};
...
//=============================================================================
	...
	Window_EnemyBookStatus.prototype.initialize = function(x, y, width, height) {
		...
		if(!PictureFitting) {
			...
			this._enemySprite.y = width / 6 + this.lineHeight();
		}
	...
	Window_EnemyBookStatus.prototype.standardFontSize = function() {
        return StandardFontSize || 18;
    };

    Window_EnemyBookStatus.prototype.standardPadding = function() {
        return StandardPadding || 6;
    };

	Window_EnemyBookStatus.prototype.lineHeight = function() {
		return this.standardFontSize() + this.standardPadding();
	};
...
// Version 1.11で復活
	...
	Window_EnemyBookStatus.prototype.update = function() {
		...
		if (this._enemySprite.bitmap) {
			...
			var contentsWidth = this.contents.width / 2;
			var contentsHeight = this.lineHeight() * 5 + this.textPadding();
			var scaleX = 1;
			var scaleY = 1;
			var scale = 1;
			if(!PictureFitting) {
			...
			if(!PictureFitting) {
				...
				if (bitmapWidth > contentsWidth) {
					scaleX = contentsWidth / bitmapWidth;
					//console.log("bitmapWidth(+"bitmapWidth"+) > contentsWidth / 2");
				}
				if (bitmapHeight > contentsHeight) {
					scaleY = contentsHeight / bitmapHeight;
				}
				scale = scaleX;
				if(scaleX > scaleY) {
					scale = scaleY;
	...
	Window_EnemyBookStatus.prototype.refresh = function() {
		...
		var disp_pop_flg = false;
		if(isUnknownEnemy) {
			if(DispPopData && DispUnknownPopData) {
				disp_pop_flg = true;
			}
		} else {
			if(DispPopData) {
		...
		if (this.isAllEnemies && !$gameSystem.isInEnemyBook(enemy.enemy())) {
			...
			this.resetTextColor();
			this.drawText(UnknownEnemy, x, y, columnWidth);
			if(DispUnknownPopData) {
				y = lineHeight * 8 + this.textPadding();
				// y = lineHeight * 2;
				this.drawPopMap(enemy, column1x, y, columnWidth, isUnknownEnemy);
			}
			if(DispUnknownDropItems) {
				y = lineHeight * 10 + this.textPadding();
				// y = lineHeight * 2;
				this.drawDropItem(enemy, column2x, y, columnWidth, isUnknownEnemy);
			}
			return;
		}
...
*/
		...
		if (dataEnemy.meta.bookLevel && DispLv) {
			this.drawText(enemy.name() + " " + TextManager.levelA + " " + dataEnemy.meta.bookLevel, x, y);
		} else {
...
*/
		...
		if(disp_pop_flg) {
			...
			this.drawPopMap(enemy, x, y, columnWidth, isUnknownEnemy);
		}
		...
		if (DispDropItems) {
			// x = column1x;
			x = column2x;
			// y = lineHeight * 5 + this.textPadding();
			y = lineHeight * 10 + this.textPadding();

			this.drawDropItem(enemy, x, y, columnWidth, isUnknownEnemy);
		}
		...
		for (var i = 0; i < 5; i++) {
			if (dispRates[i]) {
				...
				if (j % 2 == 1) {
					...
					y += lineHeight * 4 + this.textPadding();
				}
			}
		}
		if (x == column2x) {
	...
	Window_EnemyBookStatus.prototype.drawPopMap = function(enemy, x, y, columnWidth, isUnknownEnemy) {
		var lineHeight = this.lineHeight();
		var info = $dataEnemyPops.filter(function(obj){
			return obj.enemy_id == enemy.enemyId();
		});

		this.changeTextColor(this.systemColor());
		this.drawText(PopMapName, x, y, columnWidth);
		this.resetTextColor();
		y += lineHeight;

		if(info.length != 1) {
			this.drawText(NoName, x, y, columnWidth);
			return;
		}

		var mapNames = info[0]["map_names"];
		...
		mapNames.forEach(function(mapName) {
			this.drawText(mapName, x, y, columnWidth);
			y += lineHeight;
		}, this);
	};

	Window_EnemyBookStatus.prototype.drawDropItem = function(enemy, x, y, columnWidth, isUnknownEnemy) {
		var lineHeight = this.lineHeight();
		var dataEnemy = enemy.enemy();
		var iconWidth = Window_Base._iconWidth;
		var item_flg = dataEnemy.dropItems.some(function(di) {
			return di.kind > 0;
		}, this);

		this.changeTextColor(this.systemColor());
		this.drawText(DropItemName, x, y);
		this.resetTextColor();
		y += lineHeight;
		...
		for (var i = 0, l = dataEnemy.dropItems.length; i < l; i++) {
			...
			if (di.kind > 0) {
				...
				if (!isUnknownEnemy) {
					...
					if(Vitsuno && Vitsuno.Difficulty) {
						var rate = enemy.dropItemRateFromIndex(i);
					} else {
						var rate = enemy.dropItemRate();
					}

					this.drawItemNameRate(item, di.denominator, rate, x, y, columnWidth);
				} else {
					this.drawIcon(16, x, y);
					this.drawText(UnknownData, x + iconWidth, y + iconWidth / 2 - this.standardFontSize() / 2);
				}
				y += iconWidth;
			}
		}
	...
	Window_EnemyBookStatus.prototype.drawItemNameRate = function(item, denominator, rate, x, y, width) {
		var iconWidth = Window_Base._iconWidth;
		width = width || 312;
		if (item) {
			rate = 1 / denominator * 100 * rate;
			rate = "  " + Math.min(Math.floor(rate * Math.pow(10, 2)) / Math.pow(10, 2), 100) + "%";
			var iconBoxWidth = iconWidth + 4;
			this.resetTextColor();
			this.drawIcon(item.iconIndex, x + 2, y + 2);
			this.drawText(item.name, x + iconBoxWidth, y + iconWidth / 2 - this.standardFontSize() / 2, width - iconBoxWidth - this.textWidth(rate));
			this.drawText(rate, x + iconBoxWidth, y + iconWidth / 2 - this.standardFontSize() / 2, width - iconBoxWidth, "right");
		}
	};
	...
	Window_EnemyBookStatus.prototype.drawResistElement = function(x, y, w) {
		...
		for (var i=1,l=elements.length; i < l; i++) {
			...
			if (rate < 1) {
				...
				// elementNames.push($dataSystem.elements[i]);
			}
		}
		...
		if ($gameSystem.isInEnemyBook(enemy.enemy()) || this.isCheck) {
			...
			// 		this.drawText(elementNames[i], x, y);
			// 		x += dx;
			// 	}
			// } else {
...
//=============================================================================
	...
	DataManager.loadDatabase = function() {
		...
		this.loadDataFile('$dataEnemyPops', 'EnemyPops.json');
	};
