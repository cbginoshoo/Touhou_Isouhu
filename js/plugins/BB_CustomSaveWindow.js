 ...
 *
 * @param MapNameRule
 * @desc マップの設定ウィンドウで表示名が空のとき、名前の文字列をマップ名として代わりに使うかどうか
 * @type boolean
 * @on 名前を代わりに使う
 * @off 名前を使わない
 * @default true
 ...
 *
 * MapNameRule
 * マップの表示名が空の場合にマップ名を代わりに使用するかを指定
 * デフォルト : 名前を代わりに使う
 *
 *
 * ■変数に文字列を代入する方法
...
var BBSWMNR = Boolean(parameters['MapNameRule']);
//-----------------------------------------------------------------------------
// セーブデータに名前を定義
//-----------------------------------------------------------------------------
...
//add--------------------------------------------------------------------------
    info.mapname    = $gameMap.displayName() ? $gameMap.displayName() : BBSWMNR ? $gameMap.mapName() : "";
...
Game_Map.prototype.mapName = function() {
    var mapId = $gameMap ? $gameMap.mapId() : 0;
    var cnt = $dataMapInfos ? $dataMapInfos.length : 0;
