 ...
 * @default 全額でガチャを引く
 *
 * @param Button Text List
 * @desc ガチャ排出リストボタンに表示するテキストです。(FC_CardBook.jsの導入が必要)
 * 機能を使わない場合は空にしてください。
 * @default 排出リスト
 *
 ...
 *
 * Plugin Command:
 *   Gacha open                 # ガチャ画面を開きます。(単発/10連/全て)
 *   Gacha open one             # ガチャ画面を開きます。(単発のみ)
 *   Gacha open ten             # ガチャ画面を開きます。(10連のみ)
 *   Gacha add item 1           # アイテム番号1をガチャ対象に追加します。
 *   Gacha remove item 1        # アイテム番号1をガチャ対象から外します。
 *   Gacha add weapon 1         # 武器番号1をガチャ対象に追加します。
 *   Gacha remove weapon 1      # 武器番号1をガチャ対象から外します。
 *   Gacha add armor 1          # 防具番号1をガチャ対象に追加します。
 *   Gacha remove armor 1       # 防具番号1をガチャ対象から外します。
 *   Gacha clear                # 全てのガチャ対象をクリアします。
 *   Gacha amount 200           # ガチャを引くのに必要なコストを200に設定します。
 *   Gacha picup Gachabana1     # ガチャメイン画面下部のウィンドウの中央に
 *                                img/pictures/Gachabana1.pngを表示します。
 *
 *
 * Item Note:
...
(function() {
    ...
    var buttonTextList  = String(parameters['Button Text List'] || '');
    ...
    var initAmount         = Number(parameters['Required Amount'] || '100');
        ...
        if (command === 'Gacha') {
            switch (args[0]) {
                case 'open':
                    switch(args[1]) {
                        case 'one':
                            $gameSystem.gachaTypeOne();
                            break;
                        case 'ten':
                            $gameSystem.gachaTypeTen();
                            break;
                        default:
                            $gameSystem.resetGachaType();
                            break;
                    }
                ...
                case 'amount':
                    $gameSystem.setGachaAmount(Number(args[1]));
                    break;
                ...
                case 'picup':
                    $gameSystem.setPicupPicture(args[1]);
                    break;
            }
        }
    };
    ...
    Game_System.prototype.setGachaAmount = function(amount) {
        this._amount = amount;
    }

    Game_System.prototype.gachaAmount = function() {
        return this._amount;
    }
    ...
    Game_System.prototype.picupPicture = function() {
        return this._GachaPicture;
    };

    Game_System.prototype.setPicupPicture = function(filename) {
        this._GachaPicture = filename;
    };

    Game_System.prototype.gachaTypeOne = function() {
        this._GachaType = 1;
    };

    Game_System.prototype.gachaTypeTen = function() {
        this._GachaType = 2;
    };

    Game_System.prototype.resetGachaType = function() {
        this._GachaType = 0;
    };

    Game_System.prototype.gachaType = function() {
        return this._GachaType;
    };

    Game_System.prototype.clearPicupPicture = function() {
        this._GachaPicture = "";
    };
    ...
    Scene_Gacha.prototype.create = function() {
        ...
        this._helpWindow.setText(message.replace(reg, String($gameSystem.gachaAmount())));
    };
    ...
    Scene_Gacha.prototype.createCommandWindow = function() {
        ...
        this._commandWindow.setHandler('gachaList', this.commandGachaList.bind(this));
    ...
    Scene_Gacha.prototype.showPicupPicture = function() {
        if($gameSystem.picupPicture() != "" && $gameSystem.picupPicture()) {
            var bitmap = ImageManager.loadPicture($gameSystem.picupPicture());
            bitmap.addLoadListener(function() {
                var x = this.contentsWidth() / 2 - bitmap.width / 2;
                var y = this.contentsHeight() / 2 - bitmap.height / 2;
                this.contents.blt(bitmap, 0, 0, bitmap.width, bitmap.height, x, y);
            }.bind(this._dummyWindow));
        }
    };
    ...
    Scene_Gacha.prototype.commandGachaList = function() {
        $gameMap._interpreter.pluginCommand('cardbook', ['open', 'gacha', $gameSystem.picupPicture()]);
    };
    ...
    Scene_Gacha.prototype.consumeCost = function() {
        if (this.isCostTypeVariable()) {
            ...
            $gameVariables.setValue(variable, value - $gameSystem.gachaAmount());
        } else {
            $gameParty.loseGold($gameSystem.gachaAmount());
        }
    };
    ...
    Scene_Gacha.prototype.terminate = function() {
        $gameSystem.clearPicupPicture();
    ...
    Window_GachaCommand.prototype.maxCols = function() {
        switch($gameSystem.gachaType()) {
            case 1:
            case 2:
                return 2;
            default:
                return 2 + (buttonText10 ? 1 : 0) + (buttonTextAll ? 1 : 0) + (buttonTextList ? 1 : 0);
        }
    };

    Window_GachaCommand.prototype.makeCommandList = function() {
        if($gameSystem.gachaType() == 0 || $gameSystem.gachaType() == 1) {
            this.addCommand(buttonText, 'gacha', this.canGacha());
        }
        if (($gameSystem.gachaType() == 0 || $gameSystem.gachaType() == 2 ) && buttonText10) {
            this.addCommand(buttonText10, 'gacha10', this.canGacha(10));
        }
        if ($gameSystem.gachaType() == 0 && buttonTextAll) {
            this.addCommand(buttonTextAll, 'gachaAll', this.canGacha());
        }
        if (buttonTextList && Imported.FC_CardBook) {
            this.addCommand(buttonTextList, 'gachaList');
        }
    ...
    Window_GachaCommand.prototype.canGacha = function(count) {
        return this.getGachaResource() >= $gameSystem.gachaAmount() * (count || 1) && !!this._lotIsValid;
    };
    ...
    Window_GachaGet.prototype.refresh = function() {
        ...
        if (!item || !item.meta.gachaImage) {
            ...
            this._gachaSprite.bitmap = ImageManager.loadBitmap('img/pictures/', this._item.meta.gachaImage);
